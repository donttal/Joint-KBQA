...
class CRF(nn.Module):
    ...
    def __init__(self, num_tag, use_cuda=False):
        ...
        self.transitions.data[
            self.end_tag, :] = -10000  # 表示从EOS->其他标签为不可能事件, 如果发生，则产生一个极大的损失
        self.transitions.data[:, self.
                              start_tag] = -10000  # 表示从其他标签->SOS为不可能事件, 同上
    ...
    def real_path_score(self, features, tags):
        ...
        Emission score由BERT输出结合真实的tag决定，表示我们希望由输出得到真实的标签
        ...
        if self.use_cuda:
            ...
            pad_start_tags = torch.cat(
                [torch.LongTensor([self.start_tag]), tags])
            pad_stop_tags = torch.cat([tags, torch.LongTensor([self.end_tag])])
        # Transition score + Emission score
        score = torch.sum(
            self.transitions[pad_start_tags, pad_stop_tags]).cpu() + torch.sum(
                features[r, tags])
        return score
    ...
    def all_possible_path_score(self, features):
        ...
        for i in range(0, time_steps
            ...
            if i == 0:
                trans_score = self.transitions[self.start_tag, :self.
                                               start_tag].cpu()
            else:
                trans_score = self.transitions[:self.start_tag, :self.
                                               start_tag].cpu()
        ...
        forward = forward + self.transitions[:self.start_tag, self.
                                             end_tag].cpu()  # END_TAG
    ...
    def viterbi(self, features):
        ...
        back_points, index_points = [
            self.transitions[self.start_tag, :self.start_tag].cpu()
    ...
    def get_batch_path(self, inputs, output_mask):
        if not self.use_cuda:
            inputs = inputs.cpu()
            output_mask = output_mask.cpu()
        batch_all_path = []
        ...
        for ix, features in enumerate(inputs):
            ...
            best_path = torch.Tensor(best_path).long()
            best_path = padding(best_path, max_len)
            batch_all_path.append(best_path)
        # batch_best_path = torch.stack(batch_best_path, dim=0)
        return batch_all_path
...
def log_sum(matrix, dim):
    ...
    log_sum_value = clip_value + torch.log(
        torch.sum(torch.exp(matrix - clip_value), dim=dim))
